...
def show_start_interface(screen, width, height):
    '''
    show the beginning frame
    :param screen: DISPLAYSURF
    :param width: int
    :param height: int
    :return: int,the players' count
    '''
...
def show_switch_stage(screen, width, height, stage):
    ...
    screen.blit(content, rect)
    pygame.display.update()
    delay_event = pygame.constants.USEREVENT #这里是什么情况，为啥无法引用？
...
def main():
    ...
    #音乐的初始化，这个方法中有一些默认参数，设置缓存、单双声道等，它可以被调用很多次，但是第一次后除非调用了pygame.mixer.quit()，
    #否则第一次以后的调用时没办法改变第一次设置的参数的；
    #另一方面，在一些平台上要求该方法在display的初始化后调用，pygame.init()考虑到这点，但不能给这个方法传递参数；
    #为了解决这个问题，还有一个pygame.mixer.pre_init()，它里边可以传递参数，它可以在mixer的初始化和pygame的初始化之前执行
    pygame.mixer.init()

    screen = pygame.display.set_mode((630, 630))
    pygame.display.set_caption("烤地瓜的托拉斯")

    # 加载游戏开始时选择的那个黑色背景图，可以传一个图片路径，也可以传一个文件对象，但是此时似乎需要指定格式，因为
    # 这个方法不能识别出file对象的格式
    bg_img = pygame.image.load("./images/others/background.png")

    # 这里的Sound是个类，它似乎只能支持ogg和wav格式的音乐，这里加载
    # 这里似乎是发射子弹的声音，也不一定
    ...
    while not is_gameover:
        ...
        bulletsGroup = pygame.sprite.Group() #这里有重复？
